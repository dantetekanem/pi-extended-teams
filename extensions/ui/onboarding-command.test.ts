...
function setupCommand(options: {
  ...
  registerOnboardingCommand(pi, {
    ...(options.useDefaultModelLoader ? {} : { getAvailableModels }),
    loadTeamsSettings,
    createResourcePlan,
  });
  ...
  const ctx = {
    ...
    modelRegistry: {
      getAvailable: vi.fn(async () => [
        { provider: "openai", id: "registry-model", reasoning: false },
      ]),
    },
...
describe("/pi-extended-teams-onboard", () => {
  it("retains explicit thinking on scoped models separately from supported levels", async () => {
    const setup = setupCommand({
      useDefaultModelLoader: true,
      scopedModels: [{
        model: {
          provider: "anthropic",
          id: "scoped-sonnet",
    ...
    expect(prompt).toContain('"qualified": "anthropic/scoped-sonnet"');
    expect(prompt).toContain('"scopedThinking": "high"');
    expect(prompt).toContain('"supportedThinking": [');
    expect(prompt).toContain('"xhigh"');
    expect(setup.ctx.modelRegistry.getAvailable).not.toHaveBeenCalled();
  });

  it("marks registry models as unscoped when no scoped model list is present", async () => {
    const setup = setupCommand({ useDefaultModelLoader: true });

    await setup.command.handler("", setup.ctx);

    const prompt = setup.pi.sendUserMessage.mock.calls[0]![0] as string;
    expect(prompt).toContain('"qualified": "openai/registry-model"');
    expect(prompt).toContain('"scopedThinking": null');
    expect(setup.ctx.modelRegistry.getAvailable).toHaveBeenCalledOnce();
  });
