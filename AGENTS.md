...
A lead or trusted integration may explicitly save `checkpoint: { inputs: ["auth", "config"] }` and later supply `continue_from` with a precise current prompt and tier. Continuation uses a fresh recipient/run, normal claims and current permissions/checks/repair; it never revives an old mailbox or transcript. Treat saved findings, decisions and verification as history, and revalidate changed or uncertain dependency scopes, including unchanged callers. Checkpoint policy is supported in swarm defaults; continuation IDs belong on individual agents. Metadata/nested helpers cannot authorize either, and legacy terminal startup/queues reject them. Use `/agents-checkpoints list` or `/agents-checkpoints delete <id>` as lead; retirement prevents future loading but does not erase independent reports or already delivered context. See README.md for retention, failure handling and measured limits.
