...
interface OnboardingAvailableModel extends AvailableRegisteredModel {
  scopedThinking?: string | null;
}
...
async function loadAvailableModelsForOnboarding(ctx: any): Promise<OnboardingAvailableModel[]> {
  const available = Array.isArray(ctx.scopedModels) && ctx.scopedModels.length > 0
    ? ctx.scopedModels.map((entry: any) => ({
      model: entry.model,
      scopedThinking: typeof entry.thinkingLevel === "string" ? entry.thinkingLevel : null,
    }))
    : (await ctx.modelRegistry.getAvailable()).map((model: any) => ({
      model,
      scopedThinking: null,
    }));
  return available.map(({ model, scopedThinking }: { model: any; scopedThinking: string | null }) => ({
...
function buildInventory(options: {
  ctx: any;
  models: OnboardingAvailableModel[];
  settings: PiExtendedTeamsSettings;
  ...
  const availableModels = options.models
    .map((model) => ({
      ...
      scopedThinking: model.scopedThinking ?? null,
    }))
...
export function buildOnboardingPrompt(inventory: OnboardingInventory): string {
  ...
  return [
    ...
    "",
    "The availableModels.scopedThinking value is the explicit thinking level selected by Pi for that scoped model pattern; null means the model came from unscoped registry discovery. Keep it separate from supportedThinking, which lists model capabilities.",
    "",
    ...
    "5. Updating pi-extended-teams: inspect packageRegistration.scope and packageRegistration.source before giving update advice. If scope is `temporary`, the package came from `pi -e` for this run and is not in persisted global or project package settings, so `pi update --extension <source>` cannot target it. Do not suggest that command; explain that the user can rerun `pi -e <source>` with the desired source, or separately authorize `pi install <source>` when they want a persisted package. For a persisted unpinned npm source, show `pi update --extension <source>`. For a version-pinned npm source, show `pi install <source-with-new-version>` because package updates do not move it. For any persisted Git source, show `pi update --extension <source>`: Pi fetches and resets its configured ref, so moving branches and tags may advance. Reserve `pi install <source-with-new-ref-or-commit>` for changing the configured ref or commit. Explain that `pi update --extensions` updates all persisted packages. For a local path, explain that the checkout must be updated separately and then `/reload` should be run. If registration data is missing, tell the user to run `pi list` and do not guess the source.",
    "",
